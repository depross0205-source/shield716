...
# 1. 系統基礎配置 (防禦性架構)
# ==========================================
st.set_page_config(page_title="矛與盾 9.20", page_icon="🛡️", layout="wide")
st.title("🛡️ 矛與盾 v9.20 終極穩定系統 ⚔️")
...
def normalize_factors(df):
    ...
    # 映射配置 (拆解以防截斷)
    m = {}
    m['SP500'] = ['SP500', 'VOO', 'PRICE', '收盤', 'CLOSE']
    m['VIX'] = ['VIX', '恐慌', '^VIX']
    m['SP500EW'] = ['RSP', 'EW', '等權重']
    m['HY_SPREAD'] = ['SPREAD', 'HY_SPREAD', '利差']
    m['TIPS_10Y'] = ['TIPS', 'TIPS_10Y', '實質利率']
    m['CAPE'] = ['CAPE', '席勒', '本益比']
    ...
    for col in df.columns:
        if col not in list(res.columns) + [d_col]:
            res[col] = df[col]
...
st.caption("v9.20 Anti-Truncation Edition | 針對長行代碼截斷優化")

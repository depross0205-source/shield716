...
# 1. 系統環境相容性設定
# ==========================================
st.set_page_config(page_title="矛與盾 7.75 參數版", page_icon="🛡️", layout="wide")
st.title("🛡️ 矛與盾 7.75 核心參數自定義系統 ⚔️")

def safe_divider():
    """修復舊版 Streamlit 不支援 divider 的問題"""
    try: st.divider()
    except: st.markdown("---")
...
def normalize_factors(df):
    """強力識別因子，保留所有原始數據，Close 參照 SP500"""
...
def get_web_data(start_date, end_date):
    """聯網獲取最新補丁數據 (修復 TypeError: start_d)"""
    try:
...
st.sidebar.header("🛡️ 2. 熔斷機制調整 (自定義)")
with st.sidebar.expander("熔斷觸發條件設定", expanded=True):
    melt_loss_pct = st.slider("帳面虧損超過此比例則熔斷 (%)", -30, -5, -15) / 100
    melt_sma_period = st.number_input("價格低於此均線則熔斷 (週)", value=200, min_value=10)
    melt_vix_threshold = st.slider("VIX 高於此數值則熔斷", 20, 60, 40)

st.sidebar.header("⚙️ 3. 買進訊號參數")
confirm_w = st.sidebar.slider("訊號連續確認週數", 1, 5, 1)
with st.sidebar.expander("RSI 加碼門檻設定"):
    rsi_speed = st.slider("加速 (2x) RSI 門檻", 30, 55, 45)
    rsi_extra = st.slider("超賣加碼 (4x) RSI 門檻", 20, 45, 35)
    rsi_melt_buy = st.slider("熔斷中允許買入 RSI", 10, 40, 30)
...
if st.sidebar.button("🚀 執行強力數據對齊與回測", type="primary"):
    ...
    if up_file:
        ...
        # 解決 ValueError: Date label is not unique
...
if 'master' in st.session_state:
    ...
    df['RSI'] = get_rsi(df['Close'], rsi_p)
    df['SMA_Custom'] = df['Close'].rolling(window=melt_sma_period, min_periods=1).mean()
    ...
    df['M_Sig'] = (df['RSI'] < rsi_melt_buy).rolling(window=confirm_w).sum() == confirm_w
    ...
    with t1:
        ...
        # 核心：自定義熔斷判定
        is_melt = (p_loss < melt_loss_pct) or (latest['Close'] < latest['SMA_Custom']) or (latest['Vix'] > melt_vix_threshold)
        ...
        safe_divider()
        if is_melt:
            st.error(f"🔴 目前階段：熔斷中 (虧損 < {melt_loss_pct:.0%} 或 價 < SMA{melt_sma_period} 或 VIX > {melt_vix_threshold})")
            if latest['M_Sig']: st.warning(f"💡 加碼提醒：RSI 達標 {rsi_melt_buy}，可單次投入 {base_dca_amt*2/10000:.0f} 萬")
        else:
            if latest['E_Sig']: st.warning(f"🔥 目前階段：超賣爆買 ({base_dca_amt*4/10000:.0f} 萬)")
            elif latest['S_Sig']: st.warning(f"🟡 目前階段：提速扣款 ({base_dca_amt*2/10000:.0f} 萬)")
            else: st.success(f"🟢 目前階段：基礎扣款 ({base_dca_amt/10000:.0f} 萬)")
    ...
    with t2:
        ...
        for i, row in df.iterrows():
            p, r, dd, s_sig, e_sig, m_sig, v, sma = row['Close'], row['RSI'], row['DD'], row['S_Sig'], row['E_Sig'], row['M_Sig'], row['Vix'], row['SMA_Custom']
            ...
            if row['Date'].month != curr_m:
                ...
                melt_bt = (p_loss_bt < melt_loss_pct) or (p < sma) or (v > melt_vix_threshold)
                amt = 0

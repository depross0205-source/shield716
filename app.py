...
# 1. 系統基礎配置 (防禦性架構)
# ==========================================
st.set_page_config(page_title="矛與盾 9.50", page_icon="🛡️", layout="wide")
...
def get_web_data(start_d, end_d):
    ...
    try:
        ...
        w_df['SP500_Web'] = s['Close'] * 0.9 # VOO 換算
...
if st.sidebar.button("🚀 執行強力數據整合", type="primary"):
    try:
        ...
        st.success(f"✅ 數據載入成功！共 {len(st.session_state['master_df'])} 筆資料，請至右側查看。")
    except Exception as e:
